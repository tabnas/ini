...
const p = (src: string) => new Tabnas().use(jsonic).use(Ini).parse(src)
...
describe('prototype-pollution', () => {
  ...
  test('section named __proto__ does not reach Object.prototype', () => {
    const out = p('[__proto__]\npwned=X\n')
    noGlobalPollution()
    assert.equal(JSON.stringify(out), '{"__proto__":{"pwned":"X"}}')
  })

  test('key named __proto__', () => {
    const out = p('__proto__=X\n')
    noGlobalPollution()
    assert.equal(JSON.stringify(out), '{"__proto__":"X"}')
  })

  test('key named __proto__ inside a section', () => {
    const out = p('[a]\n__proto__=X\n')
    noGlobalPollution()
    assert.equal(JSON.stringify(out), '{"a":{"__proto__":"X"}}')
  })
