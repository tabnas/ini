...
| [`scripts/`](scripts/) | `fetch-ini-corpus.{sh,js}` + `build-ini-corpus.js` — clone the four upstreams at their pinned SHAs into the gitignored `test/corpus/upstream/` and regenerate `test/corpus/ini-corpus.json` from them. A regeneration/audit tool, **not** a test prerequisite: nothing in the test path runs it, and running it rewrites a committed file. |
