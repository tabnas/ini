# libtabnasini — the ini parser as a C ABI

<!-- tabnas-clib-template: v1 — stamped by admin tasks/adopt-clib.sh;
     edit the template and re-stamp, not this file. -->

The ini format parser as a C shared library, so languages with no
tabnas port can parse and validate ini input. This is one of the
...
| Function | Returns |
|---|---|
| `tabnas_version()` | `{"ok":true,"lib":"libtabnasini","format":"ini","template":"v1"}` |
...
// link against libtabnasini; every call returns a JSON []u8 to free with
// c.tabnas_free.
```
