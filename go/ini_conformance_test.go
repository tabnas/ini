...
// manifest, divergence list, canonical results and rules as
...
const missingCanonical = "canonical TypeScript results not found at " +
	"test/corpus/ini-canonical.json.\n\n" +
	"It records what the canonical runtime makes of every corpus " +
	"document this dialect reads differently from the npm/ini oracle, " +
	"and all three conformance suites read it. Regenerate it with " +
...
// loadConformanceCanonical reads the canonical TypeScript result for
// every divergent document, from the file the TypeScript and Rust
// suites read.
//
// A divergent document reads differently from the npm/ini oracle, so the
// oracle cannot say whether this port read it correctly. Asserting only
// that it DIFFERS leaves any third value green, which is 12 of the 30
// valid documents parsed and then not measured. The file is generated by
// scripts/build-ini-canonical.js and committed, because a Go test cannot
// run Node.
func loadConformanceCanonical(t *testing.T) map[string]any {
	t.Helper()
	path := filepath.Join("..", "test", "corpus", "ini-canonical.json")
	raw, err := os.ReadFile(path)
	if err != nil {
		t.Fatal(missingCanonical)
	}
	var canonical map[string]any
	if err := json.Unmarshal(raw, &canonical); err != nil {
		t.Fatalf("test/corpus/ini-canonical.json is not readable JSON: %v", err)
	}
	return canonical
}
...
func TestConformanceValidDocumentsMatchOracle(t *testing.T) {
	m := loadConformanceManifest(t)
	canonical := loadConformanceCanonical(t)
	for _, c := range m.Cases {
		if c.Kind != "valid" {
			continue
		}
		...
		// Differing from the oracle is not a result. The canonical
		// TypeScript is, and it is the contract every port is held to,
		// so the document is compared against it exactly.
		canon, recorded := canonical[c.Name]
		if !recorded {
			t.Errorf("%s: no canonical TypeScript result recorded. Add it "+
				"with `node scripts/build-ini-canonical.js %s`.",
				c.Name, c.Name)
			continue
		}
		if canonJSON := conformanceNorm(t, canon); gotJSON != canonJSON {
			t.Errorf("%s: differs from the oracle for the documented "+
				"reason (%q), but not in the way the canonical TypeScript "+
				"does.\n  got:       %s\n  canonical: %s",
				c.Name, why, gotJSON, canonJSON)
		}
	}
}

// TestConformanceCanonicalCoversDivergent holds the two lists together.
// An entry added to one and not the other would leave a document
// unmeasured again, or leave a stale result behind.
func TestConformanceCanonicalCoversDivergent(t *testing.T) {
	canonical := loadConformanceCanonical(t)
	for name := range conformanceDivergent {
		if _, ok := canonical[name]; !ok {
			t.Errorf("%s is divergent but has no canonical result in "+
				"test/corpus/ini-canonical.json", name)
		}
	}
	for name := range canonical {
		if _, ok := conformanceDivergent[name]; !ok {
			t.Errorf("%s has a canonical result but is not divergent; "+
				"delete the stale entry", name)
		}
	}
}
