...
// convenience Parse() rebuilds the (expensive) INI engine + grammar on every
// call instead of reusing a cached instance. Building the grammar — parsing
// the embedded grammar text, wiring the Hoover blocks, and applying the rule
// spec — dominates a parse, so a rebuild-per-call Parse() is many times slower
// than reusing one MakeJsonic() instance.
//
// Only the no-options Parse() path is cached, so this test exercises that
// path. Option-taking Parse(opts) calls deliberately still build fresh.
//
// The check is machine-INDEPENDENT: it compares Parse() against instance reuse
// on the SAME machine in the SAME run, so a slow CI box cannot make it flaky
