...
for SIBLING in parser json jsonic hoover support; do
...
# the jsonic base grammar, the JSON core it pulls in, the hoover block
# lexer, and the fixture
# runner). Those entries legitimately move whenever the sibling checkouts
# do, and exempting exactly them is what makes a full comparison usable
# here when blanket `--locked` is not.
