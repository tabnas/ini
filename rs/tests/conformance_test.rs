...
//!
//! Asserts exactly the same manifest, divergence list, canonical
//! results and rules as
...
const MISSING_CANONICAL: &str = "canonical TypeScript results not found at \
test/corpus/ini-canonical.json.\n\n\
It records what the canonical runtime makes of every corpus document this \
dialect reads differently from the npm/ini oracle, and all three conformance \
suites read it. Regenerate it with `node scripts/build-ini-canonical.js` from \
a built ts/. This suite FAILS rather than skips without it.";
...
/// read from `test/corpus/ini-canonical.json` beside the manifest.
///
/// A divergent document is one this grammar reads differently from the
/// npm/ini oracle, so the oracle cannot say whether a port read it
...
/// measured.
///
/// It is generated by `scripts/build-ini-canonical.js` and committed,
/// because neither this suite nor the Go one can run Node. All three
/// suites read this one file, so what the canonical means by those
/// twelve documents is decided ONCE. The key set is asserted to be
/// exactly `divergent`, so an entry cannot be added to one and
/// forgotten in the other.
fn canonical_typescript() -> BTreeMap<String, Json> {
    let path = repo_root()
        .join("test")
        .join("corpus")
        .join("ini-canonical.json");
    let raw = fs::read_to_string(&path).unwrap_or_else(|_| panic!("{MISSING_CANONICAL}"));
    let document: Json =
        serde_json::from_str(&raw).expect("test/corpus/ini-canonical.json is not readable JSON");
    document
        .as_object()
        .expect("test/corpus/ini-canonical.json is not an object")
        .iter()
...
fn every_divergent_document_has_a_canonical_result() {
    ...
    assert_eq!(
        recorded, expected,
        "test/corpus/ini-canonical.json does not hold exactly the divergent documents"
    );
}
