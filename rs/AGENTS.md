...
the valid corpus parsed and not measured.
`../test/corpus/ini-canonical.json` holds what `ts/src/ini.ts` produces
for each one, generated by `../scripts/build-ini-canonical.js` and
committed because neither this suite nor the Go one can run node, and
`valid_documents_match_the_oracle` compares against that as well. The
key set is asserted to be exactly the divergence list, so neither can be
changed without the other.

All three halves read that one file, so what the canonical means by
those twelve documents is decided once.
