...
# the Rust crate takes them as path dependencies on sibling checkouts.

.PHONY: all build test clean embed build-ts build-go build-rs \
        test-ts test-go test-rs clean-ts clean-go clean-rs \
        publish-ts publish-go version-rs tags-go reset \
        prose prose-counts
...
# Write ini-grammar.jsonic into ts/src/ini.ts, go/ini.go and
...
publish-go: test-go
	...
	sed -i.bak 's/^const VERSION = ".*"/const VERSION = "$(V)"/' go/ini.go
	rm -f go/ini.go.bak
	git add go/ini.go
	git commit -m "go: v$(V)"
	git tag go/v$(V)
...
# --- Rust (crate in rs/) ---
#
# `embed` first, the same ordering npm's own `build` script gives
# build-ts. rs/src/lib.rs carries the grammar as GRAMMAR_TEXT, the same
# jsonic text the other two embed, and the crate parses it at load; so
# without this, a focused build after an edit to ini-grammar.jsonic
# compiles the previous text and succeeds. The drift tripwire is a test,
# and a build target does not run it.
build-rs: embed
...
# ts/package.json. A release that bumps the TypeScript and Go sites and
# forgets these fails that test.
#
...
# not accept a path dependency, so tabnas-ini is not published. Only the
